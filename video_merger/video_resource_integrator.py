...
def integrate():
    ...
    for input_pc_name in input_pc_names:
        ...
        for input_fullname in input_fullnames:
            ...
            if re.fullmatch(r"pc\d_\d{8}_\d{6}.(avi|mp4|mkv|wmv)", input_basename) is not None:
                output_basename = input_basename

            # Windows標準のカメラアプリのファイルをリネームする
            if re.fullmatch(r"WIN_\d{8}_\d{2}_\d{2}_\d{2}_Pro.(avi|mp4|mkv|wmv)", input_basename) is not None:
